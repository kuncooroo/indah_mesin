...
export function ProfileAuthPanel({ googleEnabled }: { googleEnabled: boolean }) {
      ...
      <div className="mx-auto flex max-w-md flex-col">
        <div className="mb-8 text-center">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-xl bg-white shadow-sm">
            <Ms name="precision_manufacturing" className="text-[48px] text-primary" />
          </div>
        ...
        <section className="rounded-xl bg-surface-container-low p-4 shadow-sm">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            >
              ...
              <Ms name={mode === "login" ? "arrow_forward" : "person_add"} />
            </button>
          </form>
          ...
          {!googleEnabled ? (
            <p className="mt-2 text-center text-xs text-on-surface-variant">
              Google login requires OAuth credentials in the server environment.
            </p>
          ) : null}
        </section>
      </div>
